...
- **Database Restore (Preview)** — Choose **Restore...** from a server or database in Object Explorer to restore from a backup file. Pick a target database and restore mode (full, schema-only, or data-only), preview the generated command, and watch live progress with the option to cancel. ([#257](https://github.com/microsoft/vscode-pgsql/issues/257))
